-- Vyzh`dra the Exiled (162232) for Cursed cycle in Ssratemple
...
function event_spawn(e)
    total_time = 0		--tracks cumulative spawn time of Exiled.  He will depop after 25 min unegaged time as noted above
    ...
    elseif e.timer == "DT" then		--DT SCRIPT ROUTINE STARTS HERE
        if e.self:IsEngaged() then
            ...
            if target:DivineAura() then		--DA check routine
                local DA_spell_table = {	130,	--Divine Barrier
                                            199, 	--Harmshield
                                            207,	--Divine Aura
...
function event_death_complete(e)
    ...
    eq.set_global('cursed_progress', '2', 3, 'D3')
    eq.unique_spawn(162206, 0, 0 , -38, -10, -222) -- spawn Vyzh`dra the Cursed
end
